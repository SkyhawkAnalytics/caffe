    ...
    if layer.type == 'Convolution' or layer.type == 'Deconvolution':
        ...
        node_label = '"%s%s(%s)%skernel size: %d%sstride: %d%spad: %d"' %\
                     (layer.name,
                      ...
                      layer.type,
                      separator,
## YY: 10Feb2017: remove '_values' this due to error                      layer.convolution_param.kernel_size[0] if len(layer.convolution_param.kernel_size._values) else 1,
                      layer.convolution_param.kernel_size[0] if len(layer.convolution_param.kernel_size) else 1,
                      separator,
#YY same as above                      layer.convolution_param.stride[0] if len(layer.convolution_param.stride._values) else 1,
                      layer.convolution_param.stride[0] if len(layer.convolution_param.stride) else 1,
                      separator,
# YY same                      layer.convolution_param.pad[0] if len(layer.convolution_param.pad._values) else 0)
